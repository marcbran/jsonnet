# terraform-provider-cloudflare

> Terraform provider cloudflare

- [Inlined Code](https://github.com/marcbran/jsonnet/blob/terraform-provider/registry.terraform.io/cloudflare/cloudflare/terraform-provider-cloudflare/main.libsonnet): Inlined code published for usage in other projects

## Installation

You can install the library into your project using the [jsonnet-bundler](https://github.com/jsonnet-bundler/jsonnet-bundler):

```shell
jb install https://github.com/marcbran/jsonnet/terraform-provider-cloudflare@terraform-provider/registry.terraform.io/cloudflare/cloudflare
```

Then you can import it into your file in order to use it:

```jsonnet
local cloudflare = import 'terraform-provider-cloudflare/main.libsonnet';
```

## Description

