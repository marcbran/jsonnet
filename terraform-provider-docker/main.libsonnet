...
local provider(rawConfiguration, configuration) = {
  ...
  resource: {
    ...
    buildx_builder(name, block): {
      local resource = blockType.resource('docker_buildx_builder', name),
      _: resource._(block, {
        append: build.template(std.get(block, 'append', null)),
        bootstrap: build.template(std.get(block, 'bootstrap', null)),
        buildkit_config: build.template(std.get(block, 'buildkit_config', null)),
        ...
        use: build.template(std.get(block, 'use', null)),
      }),
      append: resource.field(self._.blocks, 'append'),
      bootstrap: resource.field(self._.blocks, 'bootstrap'),
      buildkit_config: resource.field(self._.blocks, 'buildkit_config'),
      ...
      use: resource.field(self._.blocks, 'use'),
    },
    config(name, block): {
    ...
    container(name, block): {
      ...
      _: resource._(block, {
        ...
        cpu_period: build.template(std.get(block, 'cpu_period', null)),
        cpu_quota: build.template(std.get(block, 'cpu_quota', null)),
      ...
      cpu_period: resource.field(self._.blocks, 'cpu_period'),
      cpu_quota: resource.field(self._.blocks, 'cpu_quota'),
